...
class TestUserService(unittest.TestCase):
    ...
    def test_login_with_valid_credentials(self):
        user = self.user_service.login(self.user_esimerkki.username, self.user_esimerkki.password)
        self.assertEqual(user.username, self.user_esimerkki.username)
    
    def test_login_with_invalid_credentials(self):
        self.assertRaises(LoginError,lambda: self.user_service.login('invaliduser', 'invalidpassword'))
