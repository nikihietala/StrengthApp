...
class ExerciseListView:
    ...
    def _initialize(self):
        ...
        deadlift_button = tk.Button(master=self._frame, text="DEADLIFT", command=self._press_deadlift)
        deadlift_button.pack(pady=2, side=tk.TOP, anchor=tk.NW)

        bench_press_button = tk.Button(master=self._frame, text="BENCH PRESS", command=self._press_bench_press)
        bench_press_button.pack(pady=2, side=tk.TOP, anchor=tk.NW)

        shoulder_press_button = tk.Button(master=self._frame, text="SHOULDER PRESS", command=self._press_shoulder_press)
        shoulder_press_button.pack(pady=2, side=tk.TOP, anchor=tk.NW)

        pull_up_button = tk.Button(master=self._frame, text="PULL UP", command=self._press_pull_up)
